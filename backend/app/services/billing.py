...
def get_usage(session: Session, company_id: uuid.UUID) -> dict:
    ...
    # AnalysisUsageEvent, NOT AnalysisJob — analysis_jobs rows are deleted with
    # their project, so counting them would let a Free org reset its quota by
    # deleting and recreating a project. The branch this came from predates that
    # fix; keeping master's model here is the point of the rebase.
