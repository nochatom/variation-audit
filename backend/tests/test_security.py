...
# -- require_any_org_admin guard -------------------------------------------
def test_require_any_org_admin_allows_admin_of_one_org():
    user = _user()
    m = _membership(user.id, uuid.uuid4(), MembershipRole.admin)
    assert require_any_org_admin(user, FakeSession(results=[FakeResult(scalar=m)])) is user


def test_require_any_org_admin_allows_admin_of_multiple_orgs():
    user = _user()
    m = _membership(user.id, uuid.uuid4(), MembershipRole.admin)
    # With our hardening (.limit(1)), only one row is returned/processed, so
    # even with multiple memberships require_any_org_admin handles it gracefully.
    assert require_any_org_admin(user, FakeSession(results=[FakeResult(scalar=m)])) is user
...
def test_require_any_org_admin_rejects_non_admin():
    user = _user()
    with pytest.raises(HTTPException) as ei:
        require_any_org_admin(user, FakeSession(results=[FakeResult(scalar=None)]))
    assert ei.value.status_code == 403
