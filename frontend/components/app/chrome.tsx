        ...
        {items.map((n) => {
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] font-medium transition-[background-color,color] duration-150 ease-out ${
                active ? "bg-ip-navy/[0.07] font-semibold text-ip-navy" : "text-ip-ink-2 hover:bg-ip-card-2 hover:text-ip-ink"
              }`}
            >
              <span className={`transition-colors duration-150 ${active ? "text-ip-navy" : "text-ip-ink-3 group-hover:text-ip-ink-2"}`}>{n.icon}</span>
              {n.label}
            </Link>
          );
        })}
          ...
          <div className="animate-scale-in absolute right-0 top-11 z-20 w-80 origin-top-right overflow-hidden rounded-lg border border-ip-line bg-ip-card shadow-ip-pop">
            <div className="flex items-center justify-between border-b border-ip-line px-3.5 py-2.5">
              <span className="inline-flex items-center gap-2 text-[13px] font-semibold text-ip-ink">
                Notifications
                {unread > 0 && (
                  <span className="rounded-pill bg-ip-orange/12 px-1.5 py-px text-[10px] font-bold tabular-nums text-ip-orange">{unread}</span>
                )}
              </span>
